...
CLEAN_COMMAND=""
export RELEASE_OS_TITLE=BlissOS
...
function displayHelp() {
    ...
    echo "--production           Disable Test Build watermark"
    echo ""
    echo "Launcher Options:"
    echo "-s, --smartdock        Enable smartdock"
    echo "-k, --kiosk            Enable kiosk launcher"
    echo "--restrictedlauncher   Enable restricted launcher"
    echo "--restrictedlauncherpro   Enable restricted launcher pro"
    ...
    echo "--perdisplayfocusime      Enable per display focus with IME"
...
if [[ $(echo ${USE_SMARTDOCK} ${USE_BLISS_KIOSK_LAUNCHER} ${USE_BLISS_GAME_MODE_LAUNCHER} ${USE_BLISS_RESTRICTED_LAUNCHER} ${USE_BLISS_RESTRICTED_LAUNCHER_PRO} ${USE_BLISS_GARLIC_LAUNCHER} | grep -c "true") -gt 1 ]]; then
    echo -e "\e[31m!!WARNING!!\e[0m"
