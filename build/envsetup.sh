...
function blissify()
{
	...
	do
	  ...
	  # Normal option processing
		...
		-d | --devclean)
		  ...
		  ;;
		-v | --vanilla)
		  echo "Building as vanilla **DEFAULT**"
		  export BLISS_BUILD_VARIANT=vanilla
		  ;;
		-g | --gapps)
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "No such item in brunch menu. Try 'breakfast' or verify your products is added to AndroidProducts.mk"
        return 1
    fi
    return $?
}
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/bliss/vars/aosp_target_release
...
function blissrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "Bliss Gerrit Rebase Usage: "
